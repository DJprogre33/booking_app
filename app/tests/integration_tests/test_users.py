...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"email": "user2@example.com", "password": "user2"}, 200, "new_user2@example.com", "new_user2"),
    ],
    indirect=["async_client_from_params"]
)
async def test_update_me(
    ...
    if response.status_code == 200:
        ...
        await async_client_from_params.put(
            "/v1/users/me", json={"email": new_email.lstrip("new_"), "password": new_password.lstrip("new_")}
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"email": "user2@example.com", "password": "user2"}, 200, "user2@example.com", "user2", "user")
    ],
    indirect=["async_client_from_params"]
)
...
@pytest.mark.parametrize(
    ...
    [
        ({"email": "user2@example.com", "password": "user1"}, 401, None, None, None, None),
        ({"email": "user3example.com", "password": "user2"}, 401, None, None, None, None),
        ({"email": "user2@example.com", "password": "user2"}, 403, None, None, None, None),
        ({"email": "owner2@example.com", "password": "owner2"}, 403, None, None, None, None),
        (
...
async def test_update_user_from_superuser(
    ...
    response = await async_client_from_params.put(
        f"/v1/users/{user_id}", json={"email": new_email, "password": new_password, "role": role}
    )
    assert response.status_code == status_code
