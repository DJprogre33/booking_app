...
async def get_users_list(
    ...
    users = await UsersService().get_users_list(
        transaction_manager=transaction_manager, offset=offset, limit=limit
    )
    logger.info(
        "Successfully got users list", extra={"total_users": len(users) if users else 0}
    )
    return users


@router.get(
    "/me", response_model=SUserResponse, responses={401: {"model": SExstraResponse}}
)
@version(1)
async def return_me(
...
async def get_user_from_superuser(
    ...
    user = await UsersService().get_user(
        transaction_manager=transaction_manager, user_id=user_id
    )
    logger.info(
        "Succesfully got user",
