...
@router.post(
    ...
    responses={409: {"model": SExstraResponse}},
)
@version(1)
async def register_user(user_data: SUserRegister, transaction_manager: TManagerDep):
    """Register a new user"""
...
@router.post(
    "/login", response_model=SUserResponse, responses={401: {"model": SExstraResponse}}
)
@version(1)
...
async def logout_user(
    request: Request, response: Response, transaction_manager: TManagerDep
) -> dict:
...
@router.post(
    "/refresh", response_model=SToken, responses={401: {"model": SExstraResponse}}
)
@version(1)
...
@router.post("/abort", responses={401: {"model": SExstraResponse}})
@version(1)
async def abort_all_sessions(
    ...
    logger.info(
        "Successfully aborted all sessions", extra={"id": user.id, "email": user.email}
    )
