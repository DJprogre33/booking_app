...
async def delete_room(
    ...
    deleted_room = await tasks_service.delete_room(
        hotel_id=hotel_id, room_id=room_id, owner_id=current_user.id
    )
...
async def add_room_image(
    ...
    updated_room = await tasks_service.add_room_image(
        hotel_id=hotel_id, room_id=room_id, room_image=room_image, owner_id=current_user.id
    )
    logger.info(
        "Succesfully uploaded a room image", extra={"image_path": updated_room.image_path}
    )
    return updated_room
...
async def delete_room_image(
    ...
    updated_room = await tasks_service.delete_room_image(
        hotel_id=hotel_id, room_id=room_id, owner_id=current_user.id
    )
